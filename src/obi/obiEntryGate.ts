...
// разворот настоящим и что дала бы отменённая попытка. Окно 10 минут: 120с оказалось мало —
// лучшие сделки (HEMI, BTR) на второй минуте выглядели невзрачно, а раскрывались позже, уже в
// зоне трейлинга. Шаг 5с (10мин/5с = ~120 замеров на сигнал) — сопоставимо по объёму лога с
// прежними 120с/2с, а для оценки, отработал ли тезис за минуты, 5с разрешения достаточно.
// Публичный эндпоинт держит такую частоту с запасом. Не в конфиге — низкоуровневая механика.
const POST_OUTCOME_DURATION_MS = 600_000;
const POST_OUTCOME_INTERVAL_MS = 5_000;
 ...
 *
 * Каждый замер OBI пишется в obi_trace.log (событие "sample"), а решения по сигналу — в
 * основной obi.log (extreme_seen / triggered / timeout); разнесение по файлам см. ObiLogger.
 ...
 * символу и пишет цену/OBI в obi_trace.log (event "post_sample"), в конце — сводку в основной
 * obi.log (event "post_summary") с экскурсиями цены относительно `referenceMid`. По входам это показывает,
